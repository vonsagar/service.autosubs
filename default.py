...
class AutoSubsPlayer(xbmc.Player):
    ...
    def onPlayBackStarted(self):
        ...
        if self.run:
            ...
            if (xbmc.Player().isPlayingVideo() and totalTime > ExcludeTime and (not xbmc.getCondVisibility("VideoPlayer.HasSubtitles")) and all(movieFullPath.find (v) <= -1 for v in ignore_words) and (isExcluded(movieFullPath)) ):
                self.run = False
                xbmc.sleep(1000)
